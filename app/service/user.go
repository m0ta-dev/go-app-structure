...
// GetUserByEmail ...
func (svc *UserWebService) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
	user, err := svc.store.User.GetUserByEmail(ctx, email)
	if err != nil {
		return nil, utils.ErrorWrap(err, "svc.user.GetUserByEmail")
	}
